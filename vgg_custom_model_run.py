...
train_images, train_labels, train_length = load_images_negative(folder = 'train', img_shape = 64, resize_shape =64,  augmentation = False, quick_load = False)
test_images, test_labels, test_length = load_images(folder = 'test', img_shape = 64, resize_shape = 64, augmentation = False, quick_load = False)
...
callback = [EarlyStopping(monitor='val_loss', patience=EPOCHS),
             ModelCheckpoint(filepath='best_vgg_custom_model.h5', monitor='val_loss', save_best_only=True)]
...
model_performance.to_csv('model_results_batch_best_vgg_custom_32_lr_E4_D_025.csv', sep=',', header=True)
